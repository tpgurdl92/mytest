					...
					<article>
						<table border="1">
							...
							<c:forEach items="${ reservations}" var="reservation">
								<c:if test="${reservation.roomNum == room.num}">
									<tr>
										<td class="roomtitle">${reservation.title }</td>
										<td class="starttime">${reservation.startTime }</td>
										<td class="endtime">${reservation.endTime }</td>
										<td class="action">
											<!-- JSTL과 EL을  이용해서 코드를 완성하시오 -->
											<c:if test="${reservation.memberId == loginId}">
												<a href="cancel?num=${reservation.num }">취소</a>
											</c:if>
										</td>
									</tr>
								</c:if>
